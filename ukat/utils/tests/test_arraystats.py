...
class TestArrayStats:
    ...
    # A random 4D array
    array = np.nan*np.ones((3, 3, 2, 2))
    array[:, :, 0, 0] = np.array([[4, 8, -1.2], [-4.8, 4, 2], [-3.8, 0, 5.0]])
    array[:, :, 0, 1] = np.array([[3, 6, -3.7], [-3.1, 2, 0], [-4.6, 4, 4.3]])
    array[:, :, 1, 0] = np.array([[2, 4, -2.2], [-1.7, 3, 3], [-2.8, 3, 3.2]])
    array[:, :, 1, 1] = np.array([[2, 2, -3.8], [-0.9, 4, 5], [-0.7, 1, 5.0]])

    def test_without_roi(self):
        # Calculate stats using ArrayStats
        stats = ast.ArrayStats(self.array).calculate()

        # Calculate "gold standard" (gs) stats
        gs_4D = calculate_stats(self.array.flatten())
        gs_3D_0 = calculate_stats(self.array[:, :, :, 0].flatten())
        gs_3D_1 = calculate_stats(self.array[:, :, :, 1].flatten())
        gs_2D_0 = calculate_stats(self.array[:, :, 0, 0].flatten())
        gs_2D_1 = calculate_stats(self.array[:, :, 0, 1].flatten())
        gs_2D_2 = calculate_stats(self.array[:, :, 1, 0].flatten())
        gs_2D_3 = calculate_stats(self.array[:, :, 1, 1].flatten())

        gs_list = [gs_4D, gs_3D_0, gs_3D_1, gs_2D_0, gs_2D_1, gs_2D_2, gs_2D_3]

        # Perform tests without ROI
        self.group_tests(stats, gs_list)

    def test_with_roi(self):
        # Simple ROI
        # roi = np.array([[0, 0, 0], [1, 1, 1], [0, 0, 0]], dtype=bool)
        # roi = np.transpose(np.tile(roi, (2, 2, 1, 1)), (2, 3, 0, 1))

        # "Random" ROI
        roi = np.ones((3, 3, 2, 2), dtype=bool)
        roi[:, :, 0, 0] = np.array([[0, 0, 0], [1, 1, 1], [0, 0, 0]])
        roi[:, :, 0, 1] = np.array([[1, 0, 0], [1, 0, 0], [1, 0, 0]])
        roi[:, :, 1, 0] = np.array([[0, 1, 1], [0, 0, 0], [0, 0, 0]])
        roi[:, :, 1, 1] = np.array([[1, 1, 0], [1, 0, 0], [0, 0, 1]])

        # Calculate stats using ArrayStats
        stats_roi = ast.ArrayStats(self.array, roi=roi).calculate()

        # Calculate "gold standard" (gs) stats
        gs_4D_roi = calculate_stats(self.array[roi])
        gs_3D_roi_0 = calculate_stats(self.array[:, :, :, 0][roi[:, :, :, 0]])
        gs_3D_roi_1 = calculate_stats(self.array[:, :, :, 1][roi[:, :, :, 1]])
        gs_2D_roi_0 = calculate_stats(self.array[:, :, 0, 0][roi[:, :, 0, 0]])
        gs_2D_roi_1 = calculate_stats(self.array[:, :, 0, 1][roi[:, :, 0, 1]])
        gs_2D_roi_2 = calculate_stats(self.array[:, :, 1, 0][roi[:, :, 1, 0]])
        gs_2D_roi_3 = calculate_stats(self.array[:, :, 1, 1][roi[:, :, 1, 1]])

        gs_list_roi = [gs_4D_roi, gs_3D_roi_0, gs_3D_roi_1,
                       gs_2D_roi_0, gs_2D_roi_1, gs_2D_roi_2, gs_2D_roi_3]

        # Perform tests with ROI
        self.group_tests(stats_roi, gs_list_roi)

    def group_tests(self, stats, gs_list):
        """
        Perform tests that compare outputs of ArrayStats.calculate() and the
        calculate_stats() function above.
    ...
    def test_bad_array_dimensions(self):
        array_1D = np.array([1])
        with pytest.raises(ValueError):
            ast.ArrayStats(array_1D)

        array_5D = np.ones((2, 2, 2, 2, 2))
        with pytest.raises(ValueError):
            ast.ArrayStats(array_5D)
