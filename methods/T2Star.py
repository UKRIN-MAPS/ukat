...
class T2Star():
    ...
    def T2StarJoao(self):
        ...
        t2Star : 3D array
            ...
            the function with T2* measured in seconds.
        r2star : 3D array
        """
        try:
            self.pixelArray[self.pixelArray == 0] = 1E-10
            ...
            with np.errstate(invalid='ignore', over='ignore'):
                noise = np.sum(self.pixelArray, axis=0) /
                (numberEchoes * matrixOnes)
                sd = (np.absolute(np.sum(np.square(self.pixelArray), axis=0)
                ...
                for echo in range(numberEchoes):
                    ...
                    while not matrixIterator.finished:
                        ...
                        if self.pixelArray[echo][index] > sd[index]:
                            sigma[index] = (np.log(self.pixelArray[echo][index]
                                            / (self.pixelArray[echo][index]
                                            - sd[index])))
