...
def resizePixelArray(pixelArray, pixelSpacing, reconstPixel=None):
    """Resizes the given pixelArray, using reconstPixel as the reference
        of the resizing operation. This method assumes that the dimension
        axes=0 of pixelArray is the number of slices for np.shape > 2."""
    ...
    if len(np.shape(pixelArray)) == 2:
        pixelArray = resize(
            pixelArray, (pixelArray.shape[0] // fraction,
                         pixelArray.shape[1] // fraction), anti_aliasing=True)
    elif len(np.shape(pixelArray)) == 3:
        pixelArray = resize(pixelArray, (pixelArray.shape[0],
                                         pixelArray.shape[1] // fraction,
                                         pixelArray.shape[2] // fraction),
                            anti_aliasing=True)
    elif len(np.shape(pixelArray)) == 4:
        pixelArray = resize(pixelArray, (pixelArray.shape[0],
                                         pixelArray.shape[1],
                                         pixelArray.shape[2] // fraction,
                                         pixelArray.shape[3] // fraction),
                            anti_aliasing=True)
